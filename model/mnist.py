...
#  NODE_INDEX        Index of this node (0-based)          int
...
def main():
    ...
    nodeIndex      = int(os.getenv('NODE_INDEX', '0'))
    ...
    print(f'***** Node index: {nodeIndex} / Total peers: {minPeers}')
    print('-' * 64)
    ...
    # ── Partition dataset per node ─────────────────────────────────────────
    x_train = x_train[nodeIndex::minPeers]
    y_train = y_train[nodeIndex::minPeers]
    x_test  = x_test[nodeIndex::minPeers]
    y_test  = y_test[nodeIndex::minPeers]
    ...
    print(f"Size of training dataset (this node): {num_train_samples}")
    print(f"Size of test dataset (this node): {len(x_test)}")
    print('-' * 64)
